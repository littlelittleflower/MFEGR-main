...
class Graph(object):
    def __init__(self, triples, relations_dict, logger):
        ...
        self.inverted = self.inverted_fn
        self.invert = self.invert_fn
        self.num = len(relations_dict) // 2
        logger.info(f"The number of relations is (not including inverse relations): {self.num}")
    ...
    def inverted_fn(self, id):
        return id > self.num - 1

    def invert_fn(self, id):
        if self.inverted_fn(id):
            return id - self.num
        else:
            return id + self.num
...
class DataReader(object):
    def __init__(self, data_path: str, logger, length: int=1):
        ...
        self.event_dict = self.read_dict(data_path, mode="events")
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        head, path, tail,fuzzy_head,fuzzy_path,fuzzy_tail,fuzzy_all = pos_triple
        ...
        fuzzy_all_tri = torch.FloatTensor([fuzzy_head,fuzzy_path,fuzzy_tail,fuzzy_all])
        neg_ents = torch.from_numpy(neg_ents)
        path = torch.LongTensor(path)

        return pos_ents, neg_ents, path, subsampling_weight, self.batch_type,fuzzy_all_tri
    ...
    def collate_fn(data):
        ...
        batch_type = data[0][4]
        fuzzy_all = torch.stack([_[5] for _ in data], dim=0)
        return positive_ents, negative_ents, paths, subsample_weight, batch_type,fuzzy_all
    ...
    def two_tuple_count(self):
        ...
        init_cnt = 3
        for head, rel, tail,fuzzy_head,fuzzy_path,fuzzy_tail,fuzzy_all in self.triples:
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        head, path, tail,fuzzy_head,fuzzy_path,fuzzy_tail,fuzzy_tri = tokens
        ...
        fuzzy_all = torch.FloatTensor([fuzzy_head,fuzzy_path,fuzzy_tail,fuzzy_tri])

        return pos_ents, neg_ents, path, neg_size, self.batch_type,fuzzy_all
    ...
    def collate_fn(data):
        ...
        batch_type = data[0][4]
        fuzzy_all = data[0][5]
        return positive_ents, negative_ents, paths, neg_size, batch_type,fuzzy_all
