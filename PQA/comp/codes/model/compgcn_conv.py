...
class CompGCNConv(MessagePassing):
    def __init__(self, in_channels, out_channels, num_rels, act=lambda x: x, params=None):
        ...
        self.w_in = get_param((in_channels, out_channels))#[768,200]
    ...
    def forward(self, x, edge_index, edge_type, rel_embed):
        ...
        self.in_type, self.out_type = edge_type[:num_edges], edge_type[num_edges:]#in_type
    ...
    def message(self, x_j, edge_type, rel_embed, edge_norm, mode):
        ...
        #weight是[768,200],200就是从这里来的,[in_dim,out_dim],200是out_channels
        ...
        # print(edge_type.device)
        #变成了[edge_type,768]
        rel_emb = torch.index_select(rel_embed, 0, edge_type)#这里好像是有问题的
        # print(rel_embed.device)
        # print(edge_type.device)
        # print(rel_embed.shape)
        # print(edge_type.tolist())
        # rel_emb: (edge_num, edge_embed_dim)
        # x_j: (edge_num, node_embed_dim),这里仍然是768
        xj_rel = self.rel_transform(x_j, rel_emb)#这里有问题
...
class EventConv(MessagePassing):
    ...
    def __init__(self, in_dim, gcn_dim, role_type_dim, role_num, act=lambda x: x, params=None):
        ...
        # self.entity_linear = torch.nn.Linear(in_dim, in_dim)
    ...
    def forward(self, new_event_index, x, event_index, role_type, role_mask, entity_event_index, entity_mask,
                role_type_embed, event_embed, evt_type_emb, final_ent_list, ent_neighbors, rel_evt_idxs, evt_neighbors):
        '''
            event_embed 事件嵌入是在这里更新
        ...
        event_entity_embeds = x[batch_event_index] #这里有问题 # event_entity_embeds: (event_num, max_role_num, entity_dim)
        # print("event_entity_embeds", event_entity_embeds.shape)
        event_trigger_embeds = torch.stack([batch_event_embed for _ in range(max_role_num)],
                                           1)  # trigger_embeds: (event_num, max_role_num, trigger_dim)
        event_type_embeds = torch.stack([batch_evt_type_emb for _ in range(max_role_num)],
                                        1)  # type_embeds: (event_num, max_role_num, trigger_dim)
        ...
        # print("event_trigger_embeds", event_trigger_embeds.shape)
        concated_embeds = torch.cat(
            (new_role_type_embeds, new_event_type_embeds, new_event_entity_embeds, event_trigger_embeds), 2)
        # print("concated_embeds", concated_embeds.shape)
        output_attn = self.attn_act(self.attn_fc(concated_embeds)).squeeze(
            2) * batch_role_mask  # output_attn: (event_num, max_role_num)
        # print("output_attn", output_attn.shape)
        attn_weights = mask_softmax(output_attn, batch_role_mask)
        # print("attn_weights", attn_weights.shape)
        #正常这里是self.entity_linear(event_entity_embeds),后面可以加上，然后就是冻结了第一层的参数
        entity_info = torch.sum(attn_weights.unsqueeze(2) * torch.nn.ReLU()(event_entity_embeds),
                                1)  # event_num, init_dim
        # print("entity_info", entity_info.shape)
        event_reprs = torch.cat((entity_info, batch_event_embed, batch_evt_type_emb),
                                1)  # event_dim, 2*init_dim+role_type_dim
        ...
        entity_embeds = torch.stack([x[ent_neighbors] for _ in range(max_evt_num + 1)],
                                    1)  # (ent_num, max_evt_num+1, init_dim) 7376, 180, 768
        ...
        attn_weights_2 = self.attn_act(self.attn_fc_2(embeds_for_attn)).squeeze(
            2) * entity_mask  # (ent_num, max_evt_num+1)
