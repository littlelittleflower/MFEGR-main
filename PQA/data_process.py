...
def generate_eval_files(args, dev_file, test_file):
    ...
    with open(os.path.join(args.dir, 'events.dict'), 'r') as f:
    ...
    def gef(eval_data, entities_dict, events_dict,relations_dict, test_file, test_trivial, test_nhop):
        ...
        for line in open(test_file,encoding='utf-8'):
            ...
            rels = [str(relations_dict[i]) for i in tokens[1].split(",")]
            head_rel = int(rels[0])
            tail_rel = int(rels[len(rels) - 1])

            if head_rel in [612, 1222] and tail_rel in [612, 1222]:
                s = str(events_dict[tokens[0]])
                t = str(events_dict[tokens[2]])
            elif head_rel == 611 and tail_rel == 1221:
                s = str(events_dict[tokens[0]])
                t = str(events_dict[tokens[2]])
            elif head_rel in [612, 1222, 611] and tail_rel == 611:
                s = str(events_dict[tokens[0]])
                ...
                try:
                    ...
                    print('不要了')
            #ere_data
            # if head_rel in [18, 37] and tail_rel in [18, 37]:
            #     s = str(events_dict[tokens[0]])
            #     t = str(events_dict[tokens[2]])
            # elif head_rel == 17 and tail_rel == 36:
            #     s = str(events_dict[tokens[0]])
            #     t = str(events_dict[tokens[2]])
            # elif head_rel in [18, 37, 17] and tail_rel == 17:
            #     s = str(events_dict[tokens[0]])
            #     t = str(entities_dict[tokens[2]])
            # else:
            #     try:
            ...
            # if head_rel in [1277, 9399] and tail_rel in [1277, 9396]:
            #     s = str(events_dict[tokens[0]])
            #     t = str(events_dict[tokens[2]])
            # elif head_rel == 1118 and tail_rel == 9240:
            #     s = str(events_dict[tokens[0]])
            #     t = str(events_dict[tokens[2]])
            # elif head_rel in [1277, 9399, 1118] and tail_rel == 1118:
            #     s = str(events_dict[tokens[0]])
            #     t = str(entities_dict[tokens[2]])
            # else:
            #     s = str(entities_dict[tokens[0]])
    ...
    gef(eval_data, entities_dict,events_dict, relations_dict, test_file, test_trivial, test_nhop)
    gef(eval_data, entities_dict, events_dict,relations_dict, dev_file, dev_trivial, dev_nhop)
...
def get_id(train_file, dev_file, test_file):
        ...
        with open(input_file, "r",encoding='utf-8') as f:
            for line in f:
                ...
                for relation in relations:
                    ...
                    if relation == 'http://custom.relation/arguments':
                        event_set.add(tokens[0])
                        entity_set.add(tokens[2])
                    elif relation == 'http://custom.relation/after':
                        event_set.add(tokens[0])
                        event_set.add(tokens[2])
                    else:
                        entity_set.add(tokens[0])
                        entity_set.add(tokens[2])

    event_dict = {event: i for i, event in enumerate(sorted(list(event_set)))}
    ...
    logging.info("Number of unique entities: %s" % len(event_dict))
    logging.info("Number of unique entities: %s" % len(entity_dict))
    logging.info("Number of unique relations: %s" % len(relation_dict))
    return entity_dict, event_dict,relation_dict


def get_nhop(args, file, entities_dict,events_dict, relations_dict, mode="train"):
    ...
    with open(file, "r",encoding='utf-8') as f:
        for line in f:
            ...
            head_rel = int(relations[0])
            tail_rel = int(relations[len(relations)-1])

            if head_rel in [612, 1222] and tail_rel in [612, 1222]:
                li.append(str(events_dict[tokens[0]]))
                li.append(",".join(relations))
                li.append(str(events_dict[tokens[2]]))
            elif head_rel == 611 and tail_rel == 1221:
                li.append(str(events_dict[tokens[0]]))
                li.append(",".join(relations))
                li.append(str(events_dict[tokens[2]]))
            ...
            #     li.append(str(entities_dict[tokens[2]]))
            #ace_2005
            # if head_rel in [1277, 9399] and tail_rel in [1277, 9399]:
            #     li.append(str(events_dict[tokens[0]]))
            #     li.append(",".join(relations))
            #     li.append(str(events_dict[tokens[2]]))
            # elif head_rel == 1118 and tail_rel == 9242:
            #     li.append(str(events_dict[tokens[0]]))
            #     li.append(",".join(relations))
            #     li.append(str(events_dict[tokens[2]]))
            # elif head_rel in [1277, 9399, 1118] and tail_rel == 1118:
...
def pathquery_data_preprocess(args, train_file, dev_file, test_file):
    entities_dict, events_dict,relations_dict = get_id(train_file, dev_file, test_file)
    with open(os.path.join(args.dir, 'entities.dict'), 'w') as f:
        json.dump(entities_dict, f, indent=4)
    with open(os.path.join(args.dir, 'events.dict'), 'w') as f:
        json.dump(events_dict, f, indent=4)
    with open(os.path.join(args.dir, 'relations.dict'), 'w') as f:
        json.dump(relations_dict, f, indent=4)
    get_nhop(args, train_file, entities_dict, events_dict,relations_dict, mode="train")
    get_nhop(args, test_file, entities_dict,events_dict, relations_dict, mode="test")
